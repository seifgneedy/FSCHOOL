      ...
      <v-row dense>
        <v-col v-for="course in courses" :key="course.code">
          <v-card  color="teal accent-9" class="mx-auto" height="200" width="300"  elevation="5" shaped hover @click="goToCourse(course.code)" >
            <v-card-title class="text-h4 text--primary font-weight-bold" v-text="course.code"></v-card-title>
            <v-card-text class="text-h5 text--primary font-weight-bold" v-text="course.name"></v-card-text>
          </v-card>
        </v-col>
      </v-row>
    </v-container>
