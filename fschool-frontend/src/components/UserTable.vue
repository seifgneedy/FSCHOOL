          ...
          <v-dialog persistent v-model="dialog" max-width="800px">
            ...
            <v-card>
              ...
              <v-card-actions>
                ...
                >
                <v-spacer />
                <v-btn @click="close" class="mr-5" dark color="error"
                  >CLOSE</v-btn
                >
                <v-btn @click="addUser" class="mr-5" dark color="success"
                  >SAVE</v-btn
                >
              </v-card-actions>
            </v-card>
          ...
          <v-dialog v-model="dialogDelete" max-width="600px">
            <v-card align="center">
              <v-card-title class="text-h6">
                ...
                {{ newUser.id }} and Email : {{ newUser.email }}
              </v-card-title>
              <v-card-actions>
                <v-spacer />
                <v-btn color="blue darken-1" dark @click="closeDelete"
                  >Cancel</v-btn
                >
                <v-btn color="blue darken-1" dark @click="deleteUserConfirm"
                  >OK</v-btn
                >
                <v-spacer />
              </v-card-actions>
...
export default {
  ...
  methods: {
    ...
    async deleteUserConfirm() {
      let response;
      await AXIOS.delete(`admin/user?id=${this.newUser.id}`)
        .then((res) => {
          response = res.data;
        })
        .catch(() => {
