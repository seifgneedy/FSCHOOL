...
public class StudentServiceTest {
    ...
    private void addUsersToCourses(){
        ...
        courseList.get(0).addUser(studentList.get(0));
        courseList.get(1).addUser(studentList.get(0));
        // user 1 is only in course CSE 123
        courseList.get(0).addUser(studentList.get(1));
    ...
    public void getCoursesOfUserInASingleCourse(){
        prepareDB();
        User user = studentList.get(1);
        when(userRepository.findById(user.getId())).thenReturn(Optional.of(user));
        List<Course> courses = studentService.getCourses(user.getId());
    ...
    public void getCoursesOfUserInMultipleCourses(){
        ...
        when(userRepository.findById(user.getId())).thenReturn(Optional.of(user));
        List<Course> courses = studentService.getCourses(studentList.get(0).getId());
        assertNotNull(courses);
    ...
    public void getCourseWithCodeInDB(){
        ...
        Course course = studentService.getCourse("CSE 123");
